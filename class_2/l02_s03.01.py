...
for review_data in all_data:
	...
	if not score in reviews_per_class:
		reviews_per_class[score]=0.0
	reviews_per_class[score]+=1.0

	if not score in words_per_score:
		words_per_score[score]=0.0
	...
	for word in words:
		if not word in stop_words:
			...
			words_per_score[score]+=1.0

# p[C] -> reviews_per_class[score]
for score in reviews_per_class:
	reviews_per_class[score]/=float(len(all_data))

for score in dictionary_per_score:
	for word in dictionary_per_score[score]:
		dictionary_per_score[score][word]/=words_per_score[score]

## Here I have P[ X | C]  -> p[C | W]  = p[X|C]*p[C]/p[W]  ->  p[X|C]*p[C] 

for word in master_dictionary:
	print word
	for score in dictionary_per_score:
		if word in dictionary_per_score[score]:
		# p[C_j| w_i] =  p [w_i| c_j] * p[c_j]
			print dictionary_per_score[score][word]*reviews_per_class[score]
